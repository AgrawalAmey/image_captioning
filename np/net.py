...
class Net(object):
	...
	def __init__(self, data, **kwargs):
		...
		self.store_every = kwargs.pop('store_every', 2000)
	...
	def loss(self, features, captions):
		captions_in = np.zeros_like(captions)
		captions_out = np.zeros_like(captions)
		captions_in[:, 1:] = captions[:, :-1]
		captions_in[:, 0] = self._start
		captions_out[:, :-1] = captions[:, 1:]
		captions_out[:, 0] = self._end
		...
		temp = self.temporal_affine_h0.forward(features.reshape((N, 1, D)))
		h0 = temp.reshape((N, H))
	...
	def train(self):
		...
		for t in xrange(num_iterations):
			...
			if t % self.store_every == 0:
				cPickle.dump(self, open("net.p", "wb"))
				cPickle.dump(self.loss_history, open("loss.p", "wb"))
	...
	def sample(self, features, max_length=30):
		...
		h = self.temporal_affine_h0.forward(features.reshape((N, 1, -1)))
		h = h.reshape((N, H))
		c = np.zeros((N, H))
		...
		for i in range(max_length):
			# Word embedding
			x = self.word_embedding.forward(captions[:, i-1].reshape((N, 1)))
			x = x.reshape((N, D))
			# RNN forward
			h, c = lstm_cell.forward(x, h, c, self.params['Wx'], self.params['Wh'], self.params['b'])
			# Scores
			scores = self.temporal_affine_scores.forward(h.reshape((N, 1, H))) 
			scores = scores.reshape((N, V))
			# Add next word
